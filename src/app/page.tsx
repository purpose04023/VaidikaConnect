...
export default async function HomePage() {
  ...
  const featuredPujas = (await getPujas()).slice(0, 3);
  ...
  return (
    <div className="flex flex-col items-center text-center">
      ...
      <section className="w-full py-16 md:py-24 bg-accent/10">
        <div className="container mx-auto px-4">
          <h2 className="font-headline text-3xl md:text-4xl font-bold mb-12">Featured Programs</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {featuredPujas.map(puja => (
              <Card key={puja.id} className="overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 flex flex-col text-left">
                <Image
                    src={puja.image}
                    alt={puja.name_en}
                    width={600}
